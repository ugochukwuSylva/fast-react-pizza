      ...
      <Form method="POST">
        ...
        <div className="mb-12 flex items-center gap-5">
          <input
            ...
            name="priority"
            id="priority"
            className="flex h-6 w-6 accent-yellow-400 focus:ring focus:ring-yellow-400 focus:ring-offset-2"
            value={withPriority}
...
export async function action({ request }) {
  ...
  if (!isValidPhone(order.phone))
    errors.phone =
      "Kindly provide us with a valid phone number with which to reach you";

  if (Object.keys(errors).length > 0) return errors;

  const newOrder = await createOrder(order);

  // Do not overuse, as it reduces in-built performance optimization from redux
